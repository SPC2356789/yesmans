...
<div class="flex flex-wrap mb-4 gap-0.5 ss:gap-1 md:gap-4 justify-between">
    @if(isset($items))

        @if($items && $items->isNotEmpty())
            @foreach($items as $item)
                <a href="/{{$Slug}}/{{$item->category_slug}}/{{$secondSlug}}/{{$item->slug}}"
                        id="{{$Slug}}_{{$item->id}}"
                        class="w-[47%] lg:w-[31%]   addHot group">
                    ...
                    <div
                        ...
                        <div class="relative w-full" style="padding-top: 100%;">
                            @if(isset($tags))
                                <div class="flex flex-wrap absolute top-0 right-0 gap-1 z-10 xsm:m-2 xxx:m-1">
                                    @foreach($item->tags as  $itv)
                                        {{--                                        <button class="span_tag sm:text-sm" data-value="{{$tk}}">{{$tv}}</button>--}}
                                        <span
                                                class="xxx:px-1 ss:px-1.5 xxx:rounded-xl ss:rounded-3xl bg-white text-[#212121] text-xs flex items-center justify-center">{{$tags[$itv]}}</span>
                                    @endforeach
                                </div>
                            @endif
                            @if(isset($MediaMlt))
